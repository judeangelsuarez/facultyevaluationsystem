...
<div class="content-body">
            ...
            <div class="container-fluid">
			    ...
			    <h1 class="card-header">Select Faculty</h1>
							    <form class="settings-form" method="POST" action="#">
									<div class="mb-3">
									    ...
									    <fieldset class="form-group">
                                                     ...
                                                     <?php
                              $ysql = "SELECT *,c.facultyid as 'fac' FROM `class` c LEFT JOIN `faculty` f ON f.facultyid = c.facultyid LEFT JOIN subject s ON s.subjectid = c.subjectid LEFT JOIN `schoolyear` sy ON sy.syid = c.syid WHERE c.facultyid NOT IN (SELECT facultyid from evaluation WHERE facidno = '".$_SESSION['facidno']."')";
                            ...
                            <select class="form-control select2 facc" name="class_id">
                              ...
                              while($rowy = mysqli_fetch_assoc($yresult)){
                                ?>
                                <option value="<?php echo $rowy['class_id']; ?>"><?php echo $rowy['facultyname'].' - ('.$rowy['coursename'].') '.$rowy['subjectname'] ;?></option>
                                <?php
                              }
                              ?>
                          </select>
		                ...
		                <div class="app-card app-card-settings shadow-sm p-4">
						    <div class="app-card-body">
							    <form class="settings-form" action="controller.php" method="POST">
							    	...
							    	<input type="hidden" name="facidno" value="<?php echo $_SESSION['facidno']; ?>">
				...
				<div class="text-center">
					<?php
						if($rowteach['facultyname']==$_SESSION['fname']){
							echo '<input type="hidden" value="Self" name="evaluator_type">';
							echo '<strong><u><p>Self</p></u></strong>';
						}else{
							echo '<input type="hidden" value="'.$_SESSION['eval_type'].'" class="form-control" name="evaluator_type">';
							echo '<strong><u><p>'.$_SESSION['eval_type'].'</p></u></strong>';
						}
						?>
				</div>
				<br>
				...
				<p>Instruction: Please evaluate the faculty using the scale below. Check the radio button.</p>
					   ...
					   <p class="text-left">5 = Outstanding, 4 = Very Satisfactory, 3 = Satisfactory, 2 = Fair, 1 = Poor</p>
					</fieldset></div><br>
					<?php
...
?>
								...
								<td class="text-center">
									...
									<input type="hidden" name="criteriaid[]" value="<?php echo $rowq['criteriaid']; ?>">
				                        ...
				                        <input type="radio" required name="rate[<?php echo $rowq['questionid']; ?>]" id="qradio<?php echo $rowq['questionid'].'_'.$c ?>" value="<?php echo $c ?>">
